-- ============================================================
-- からだ鍼灸整骨院（karada鍼灸整骨院）初期コースセットアップ + slot 20分化
-- ============================================================
-- 単一 Supabase × clinic_id 分離構成のため、当該 clinic_id 直接指定。
-- 対象院:
--   clinic_settings.id = 'd3b55abc-46a6-4cbe-8198-21c0392d9a2e'
--   ※ ボール接骨院などの別 clinic 行には影響しない
--
-- 内容:
--   1. 既存コース参照中の appointments.course_id を NULL（履歴 course_name は保持）
--   2. 既存コースを全削除
--   3. slot_duration_minutes を 20 に
--   4. 新規 29 コース投入（HP料金参考、藤川先生承認待ち）
--      ・保険施術       : 初診40分 / 再診20分
...
--
-- 実適用:
--   2026-05-21 に Supabase MCP 経由で本番 (d3b55abc) に直接適用済。
--   migrate-on-build は SUPABASE_DB_URL 未設定で skip されるため、
--   このファイルは履歴目的（将来の再現性 + 別 Supabase 移管時用）。
-- ============================================================
...
BEGIN
  -- このDB に対象 clinic が無ければ no-op（ball/muscle/relaq の Supabase でも安全）
  IF NOT EXISTS (SELECT 1 FROM public.clinic_settings WHERE id = v_clinic_id) THEN
    RAISE NOTICE '[karada-setup] clinic % not found in this Supabase — skipping.', v_clinic_id;
  ELSE
    RAISE NOTICE '[karada-setup] Applying to clinic_id=%', v_clinic_id;

    -- 1. 既存コース参照の appointments を course_id NULL 化（course_name は保持）
    UPDATE public.appointments
       SET course_id = NULL
     WHERE clinic_id = v_clinic_id
       AND course_id IN (SELECT id FROM public.reservation_courses WHERE clinic_id = v_clinic_id);

    -- 2. 既存コース削除
    DELETE FROM public.reservation_courses WHERE clinic_id = v_clinic_id;
