...
import {
  ChevronLeft, ChevronDown, ChevronUp, Loader2, Save, Target, NotebookPen, CheckCircle2, Pencil,
} from "lucide-react";
...
/**
 * 採点フォーム。新規採点と、保存済み評価の修正の両方に使う。
 * assessmentId があれば「修正」モード（既存の点数・コメントを読み込んで上書き保存）。
 */
export default function AssessmentForm({
  ...
  useEffect(() => {
    (async () => {
      try {
        if (assessmentId) {
          const data = await getAssessmentEdit(assessmentId);
          if (!data) {
            setNotFound(true);
            return;
          }
          ...
          setNextGoal(a.next_goal ?? "");
          setHomework(a.homework ?? "");
  ...
  const handleSave = async () => {
    ...
    setSaving(false);
    if (res.success) {
      toast.success(isEdit ? "修正を保存しました" : "評価を保存しました");
  ...
  if (notFound) {
    return (
      <div className="p-10 text-center text-slate-500">
        <p>この評価は見つかりませんでした。</p>
        <Link href={`/admin/training/${customerId}`} className="text-emerald-700 underline text-sm mt-2 inline-block">
          患者ページへ戻る
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-2xl mx-auto pb-28">
      ...
      {isEdit && (
        <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50/70 p-3 text-sm text-amber-800">
          保存済みの評価を書き直しています。点数のチップをもう一度押すと、その項目は未採点に戻ります。
        </div>
      )}
      ...
      <div className="fixed bottom-0 left-0 right-0 md:left-64 bg-white/95 backdrop-blur border-t px-4 py-3 z-20">
        <div className="max-w-2xl mx-auto flex items-center gap-3">
          ...
          <Button onClick={handleSave} disabled={saving} className="flex-1 h-12 text-base bg-emerald-600 hover:bg-emerald-700">
            {saving ? <Loader2 className="w-5 h-5 animate-spin" /> : <><Save className="w-5 h-5 mr-1" />{isEdit ? "修正を保存する" : "保存する"}</>}
          </Button>
        </div>
      </div>
    </div>
