...
M/D(曜)  受付: 名前 HH:MM-HH:MM → 名前 HH:MM-HH:MM …  ｜ 教室・外部: 担当 HH:MM-HH:MM（方針にそういう業務がある院だけ）

ルール（厳守）:
- 受付は営業時間を絶対に途切れさせない。誰で埋めるか・補填の順番・院長を入れてよいか・教室や外部業務の担当は、上の【オーナーの方針・軸】に書かれたとおりにする。
- 方針に名前の出ていない人を勝手に割り当てない。退職・休職と書かれている人は絶対に入れない。
- どうしても埋まらない枠は「（空き）要調整」と明記して院長の判断に委ねる（無理に埋めない）。
