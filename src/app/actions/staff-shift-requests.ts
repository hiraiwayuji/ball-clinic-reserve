...
# 出力（日本語・前置き不要・本文のみ）
最初に1行サマリ。続けて営業日ごとに必ず1行ずつ、次の形式で：
M/D(曜)  受付: 名前 HH:MM-HH:MM → 名前 HH:MM-HH:MM …  ｜ はなまる: 担当 HH:MM-HH:MM

ルール（厳守）:
- 受付は 9:30-20:00 を絶対に途切れさせない。受付職（平岡・笹野・金田）で埋め、足りなければ施術者で補填し、補填順は 森藤 → 森川。
- 藤川先生（院長）は院受付にもはなまるにも基本入れない。自動で割り当てず、どうしても埋まらない枠は「（空き）要調整」と明記して先生の判断に委ねる。
- 馬場先生は日中（〜18:00）は訓練（トレーニング）業務のため院受付に入れないが、18:00以降の夜の院受付は馬場先生が基本担当。はなまるの代打もOK。
- 「はなまる教室」の担当＝月曜:花増／火・木・金:金田中心。金田が休みの代打は 平岡→馬場 の順（馬場は院受付はNGだが、はなまるはOK）。施術者（森藤・藤川・森川）は絶対にはなまるに出さない。
- はなまる担当は、はなまる時間中に加え「前後40分（院との移動時間）」も院の受付に入れない。それ以外の時間は院受付に回してよい。
- 代打でも埋まらない場合は はなまる欄に「⚠要調整」と明記（施術者では埋めない）。
- 各スタッフの出勤可能時間・休み希望は厳守する（希望外の時間に勝手に入れない）。

最後に「■ とくに注意」として、受付が手薄な日・はなまるの代打が要る日・休み希望の重なりを箇条書き。`;
