...
- 「はなまる教室」は受付職（平岡・笹野・金田）の担当。施術者（森藤・藤川・森川）は絶対にはなまるに出さない。
- はなまる担当（上の方針に時間あり）は、その時間は院の受付に入れない。前後の時間は院受付に回してよい。
- はなまる担当（金田）が休みの日は、他の受付職（平岡→笹野）で代打。受付職で埋まらない場合は はなまる欄に「⚠受付職で要調整」と明記（施術者・馬場では埋めない）。
