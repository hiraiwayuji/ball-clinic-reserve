...
ルール（厳守）:
- 受付は 9:30-20:00 を絶対に途切れさせない。受付職（平岡・笹野・金田）で埋め、足りなければ施術者で補填し、補填順は 森藤 → 藤川 → 森川。
- 馬場先生は訓練（トレーニング）業務のため、院受付にもはなまるにも入れない。受付要員として数えない。
- 「はなまる教室」は受付職の担当。施術者（森藤・藤川・森川）は絶対にはなまるに出さない。担当は金田がメイン、月曜は花増。
- はなまる担当は、はなまる時間中に加え「前後40分（院との移動時間）」も院の受付に入れない。それ以外の時間は院受付に回してよい。
- はなまる担当（金田）が休みの日は、他の受付職（花増→平岡→笹野）で代打。受付職で埋まらない場合は はなまる欄に「⚠受付職で要調整」と明記（施術者・馬場では埋めない）。
