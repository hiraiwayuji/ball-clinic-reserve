...
export async function generateSEOMeoAdvice() {
  ...
  try {
    ...
    const prompt = `
      ...
      2. ホームページURLが提供されている場合は、そのドメイン名や構成から推測される具体的改善点（タイトルタグの工夫やスマホ対応の重要性など）を必ず1つ含めてください。提供されていない場合は、一般的なHP作成のコツを伝えてください。
      3. それぞれ、明日からすぐ実行できる具体的なアクションを3つずつ（計6つ）提案してください。
      4. 近隣の競合に勝つための「差別化ポイント」を1つ指摘してください。
      5. 専門用語は避け、院長先生が実行しやすい平易な言葉で書いてください。
      6. 全体で400文字〜600文字程度で、Markdown形式で出力してください。バッジやボールドを使って強調してください。
