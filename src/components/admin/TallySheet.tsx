...
import { Loader2, Plus, Trash2, Save, RefreshCw, BarChart3, Coins, UserPlus, CalendarDays } from "lucide-react";
import {
  getTallySheet,
  saveTallySheet,
  type TallyStaff,
    ...
    <div className="p-3 sm:p-5 max-w-[1200px] mx-auto">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">当日の売上をまとめて記帳</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
        ...
        <table className="w-full text-sm border-collapse min-w-[860px]">
          <thead>
            <tr className="bg-slate-50 dark:bg-slate-900/50 text-slate-600 dark:text-slate-300">
              ...
              {columns.map((c) => (
                <th key={c.key} className="px-2 py-2 text-right font-semibold w-24 whitespace-nowrap">{c.label}</th>
              ))}
              <th className="px-2 py-2 text-right font-semibold w-24">合計</th>
              <th className="px-2 py-2 text-center font-semibold w-12">新患</th>
          ...
          <tbody>
            {rows.map((r) => {
              ...
              return (
                <tr key={r._id} className="border-t border-slate-100 dark:border-slate-700/50 hover:bg-slate-50/60 dark:hover:bg-slate-900/30">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mt-4">
        ...
        <button
          ...
          onClick={handleSave}
          disabled={saving}
          className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-700 hover:to-violet-700 text-white font-bold shadow-md active:scale-95 transition-all disabled:opacity-60"
        >
      ...
      <p className="mt-2 text-[11px] text-slate-400">
        ※ 保存するとこの日の日計表は入力内容で上書きされます。金額が入っていない行は登録されません。
      </p>
    </div>
  );
}
