...
const CLINIC_NAV_ITEMS: NavItem[] = [
  ...
  { href: "/admin/appointments", label: "予約カレンダー", icon: CalendarDays, allow: ["owner", "admin", "staff"], group: "日々の業務", description: "予約カレンダー。予約の追加・変更・休憩枠もここから" },
  ...
  { href: "/admin/attendance-list", label: "勤怠一覧・給与", icon: ListChecks, allow: ["owner"], group: "スタッフ管理", description: "全員の出勤・退勤を月ごとに一覧で見て、時給から支給額の目安を出す。時刻・休憩の修正や記録の追加もここ" },
  ...
  { href: "/admin/help", label: "使い方・困ったとき", icon: HelpCircle, allow: ["owner", "admin", "staff"], group: "ヘルプ", description: "操作マニュアルと、困ったときの連絡先" },
];
...
export type AdminNavOptions = {
  /** clinic_settings.sales_input_mode。"tally" の院は「売上記帳」を「日計表」と表示する */
  salesInputMode?: string | null;
};

export function getAdminNavItems(role: Role, options: AdminNavOptions = {}): NavItem[] {
  ...
  return source
    ...
    .map((item) => {
      if (item.href === "/admin/sales" && options.salesInputMode === "tally") {
        return { ...item, label: "日計表", description: "窓口日計表。その日の来院者の金額をまとめて記帳する画面" };
      }
      return item;
    });
}
