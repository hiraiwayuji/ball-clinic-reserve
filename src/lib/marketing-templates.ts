...
export const CAMPAIGN_INFO: Record<CampaignKey, CampaignInfo> = {
  ...
  birthday: {
    ...
    how: "ボタンを押すと、実費メニュー500円OFFのクーポンメッセージが届きます。患者さんは来院時にこのLINE画面を見せて使います。",
    tips: "設定で「誕生日のお祝いメッセージ」をONにすると、誕生日当日に自動送信もできます（有効期限は誕生日から1ヶ月）。※クーポンは実費メニューのみ対象（保険診療への値引きは不可）。",
  },
  lottery: {
    title: "来院者限定抽選会",
    who: "来院実績があり、LINE連携済みの患者さん全員",
    how: "ボタンを押すと自動で抽選が行われ、約10%の方に「実費メニュー500円引き当選」、それ以外の方に「ハズレ（また来月）」のLINEが届きます。当選者が0人の場合は1名が自動で当選になります。",
    tips: "全員に結果LINEが届くので、月1回のお楽しみイベントとして使うと再来院のきっかけになります。※割引は実費メニューのみ対象（保険診療への値引きは不可）。",
  },
  women: {
    title: "女性限定施策",
...
/**
 * ★重要（法令）★ クーポン・割引は「実費（自費）メニューのみ」対象と必ず明記すること。
 * 保険診療に値引きを適用するのは違法（一部負担金の値引きにあたる）。
 * 対象を書かない「施術料金◯◯円OFF」のような表現は使わない。
 */
const JIHI_ONLY_NOTE = "※実費（自費）メニューのみ対象です。保険診療にはご利用いただけません。";

/** 誕生月に手動でまとめて送る用（月単位）。有効期限は今月末。 */
...
/**
 * 誕生日「当日」に自動送信する用。
 * 有効期限は誕生日から1ヶ月（expiryDate に "2026年8月17日" のような文字列を渡す）。
 */
export function birthdayDayMessage(
  clinicName: string,
  customerName: string,
  expiryDate: string,
): string {
